...
from adminapp.models import employeedb, postdb, farmerdb, attendancedb,imagedb, messagedb, orderdb
...
def editemp(req, dataid):
    des = postdb.objects.all()
    emp = employeedb.objects.get(Mobile=dataid)
...
def empupdate(req, dataid):
    if req.method == "POST":
        ...
        try:
            ...
            file = employeedb.objects.get(Mobile=dataid).Image
        employeedb.objects.filter(Mobile=dataid).update(Empname=na, Designation=ds, Address=ad, Dateofbirth=dob, Gender=gr,
...
def empdelete(req, dataid):
    data = employeedb.objects.filter(Mobile=dataid)
    data.delete()
...
def editfarmer(req, dataid):
    far = farmerdb.objects.get(Mobile=dataid)
...
def farmerupdate(req, dataid):
    if req.method == "POST":
        ...
        try:
            ...
            file = farmerdb.objects.get(Mobile=dataid).Image
        farmerdb.objects.filter(Mobile=dataid).update(Cardno=cn, Name=na, Address=ad, Dateofbirth=dob, Gender=gr,
                                                    Mobile=mob, Email=em, Image=file)
        messages.success(req, "Data Updated Successfully....!")
        return redirect(dispfarmer)
def farmerdelete(req, dataid):
    data = farmerdb.objects.filter(Mobile=dataid)
    data.delete()
    return redirect(dispfarmer)
def attendance(req):
    data = employeedb.objects.all()
...
def reply(req,dataid):
    ms = messagedb.objects.get(id=dataid)
    return render(req, "replyemail.html", {'ms':ms})


def send_email(request, dataid):
    if request.method == 'POST':
        email = request.POST['email']
        subject = request.POST['sub']
        message = request.POST['message']
        
        # You can include any additional logic here, like fetching more details from the model
        ms = get_object_or_404(messagedb, id=dataid)
